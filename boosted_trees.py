...
for n_trees in [100, 200, 300, 400, 500]:
    ...
    fprs.extend(fpr)
    tprs.extend(tpr)
    n_trees.extend(n_trees * np.ones(len(tpr)))

#plot, save
n_trees = [str(d) for d in n_trees] #turn this to string for categorical colour scheme
to_plot = pd.DataFrame({"FPR": fprs, "TPR": tprs, "n_trees": n_trees})
